...
import { ExpiredSessionError, InvalidArgumentError, InvalidRoleAccessError } from "@/exceptions";
import type { ApiClientConfig, AxiosRequestConfigWithMeta } from "@/interfaces/axiosInstanceTypes";
import { attachClientTokens, attachContextHeaders, beginRequestTracking, clearStoredTokens, ensureHeaders, finalizeRequestTracking, getRefreshToken, notifyClient, setAccessToken } from "@/utils/axiosFeatures";
...
export function createApiClient(config?: ApiClientConfig): AxiosInstance {
    const { baseURL, timeout, withCredentials, validateStatus, maxRedirects, ...options } = config || {};
    ...
    instance.interceptors.response.use(
        (response) => {
            finalizeRequestTracking(pendingRequestCount, response.config as AxiosRequestConfigWithMeta);
        ...
        async (error) => {
            const originalRequest = error.config as AxiosRequestConfigWithMeta | undefined;
            finalizeRequestTracking(pendingRequestCount, originalRequest);

            const status = error.response?.status;
            if (status && status >= 400 && status < 500) {
                notifyClient(error.response?.data?.message || "Une erreur est survenue", "error");
            }
            ...
            if (status === 401 && originalRequest && !originalRequest._retry) {
                ...
                if (refreshToken) {
                    ...
                    try {
                        const newAccessToken = await refreshAccessToken(refreshToken, instance.defaults.baseURL);
                        if (newAccessToken) {
